...
const DashboardHome = () => {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			const { data } = await progressor.post(
				"leetcode/init/",
	...
	return (
		<>
			{user?.is_leetcode_connected === false && (
				<>
					{loading === false ? (
						...
						<div className="w-full h-full flex flex-col justify-center items-center">
							...
							/>
							<h3 className="my-4 text-center font-medium text-xl opacity-0 pointer-events-none visibility-none">
								Dummy text for spacing
							</h3>
							<h3 className="my-4 text-center text-lg">
								Getting your stats. This may take a while.
							</h3>
						</div>
					)}
				</>
			)}
			{user?.is_leetcode_connected === true && <div>Account Connected</div>}
		</>
	);
};
